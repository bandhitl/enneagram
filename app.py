...
if st.session_state.get("submitted"):
    ...
    summary_df = df_resp.groupby(["Type", "Category"]).mean(numeric_only=True).reset_index()
    pivot_table = summary_df.pivot(index="Type", columns="Category", values="Score").fillna(0)
    ...
    if not core_scores.empty:
        ...
        if not wing_scores.empty:
            ...
            if core_label in summary_map:
                type_summary = summary_map[core_label]
                st.markdown(f"#### จุดแข็ง: {type_summary['strength']}")
                st.markdown(f"#### ความเครียดเมื่อเผชิญปัญหา: {type_summary['stress']}")
                st.markdown(f"#### ไม่สบายใจเมื่อทำงานกับคนแบบ: {type_summary['conflict']}")
                st.markdown(f"#### ทำงานเข้าขากับคนแบบ: {type_summary['synergy']}")
